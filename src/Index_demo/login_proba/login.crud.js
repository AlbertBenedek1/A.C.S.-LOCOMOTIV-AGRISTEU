...
function uuidv4(){
 ...
 function renderUsrs(){
    ...
    for (var usr of state.usrs){     //összeollózom az adatokat
      usersHTML += `
      <div class="card">
      <p>${usr.name} , ${usr.email}, ${usr.password}, ${usr.address}, ${usr.phone}</p>
...
document.getElementById("user-form").onsubmit = function (event){  //elem hozzáadása az inputba írt értékek alapján
    ...
    var addressInput = event.target.elements.n_address.value;
    var phoneInput = event.target.elements.n_phone.value;
